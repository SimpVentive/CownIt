...
function CeoPeople({ data }: CeoPeopleProps) {
  ...
  const getLastActive = (personId: string): string => {
    ...
    const allDates = [...updates, ...achievementDates];
    ...
    <div>
      ...
      <div className="flex flex-col gap-3">
        {data.people.map((person) => {
          ...
          return (
            <div key={person.id} className="rounded-xl border border-[#e0e0e0] bg-white p-4">
              <div className="mb-3 flex items-start justify-between">
                <div>
                  <div className="mb-1 text-[13px] font-medium">{person.name}</div>
                  <div className="text-xs text-[#666]">{person.department}</div>
                </div>
                <span
